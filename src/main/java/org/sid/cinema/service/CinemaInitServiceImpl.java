...
public class CinemaInitServiceImpl implements ICinemaInitService {
    ...
    @Autowired
   	private  TicketRepository  ticketRepositoryt;
	@Override
	...
	public void initCinemas() {
		...
		villeRepository.findAll().forEach(v -> {
	        ...
	        cinema.setNombreSalles(1+(int)(Math.random()*6));
	        cinema.setVille(v);
	...
	public void initFilms() {
		...
		Stream.of("12 Homes en colaire","Forset Gump","Green Book","La ligne Verte","Le Parin","Le Signeur des anneaux").forEach( filmName  -> {
			Film film = new Film();
	...
	public void initTickets() {
		...
		projectionRepository.findAll().forEach(p -> {
			p.getSalle().getPlaces().forEach(place -> {
				...
				ticket.setPrixAchat(p.getPrix());
				ticket.setReserve(false);
				ticketRepositoryt.save(ticket);
			});
		});
